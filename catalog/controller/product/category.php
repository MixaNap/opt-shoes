		...
		if ($category_info) {
			...
			//   }
				...
				$data['categories'][] = array(
					...
					'img' => $this->model_tool_image->resize($result['image'], 80, 103),
		...
		//
				...
				if ($product_attributes) {
					foreach ($product_attributes as $attribute_group) {
						if (isset($attribute_group['attribute']) && is_array($attribute_group['attribute'])) {
							foreach ($attribute_group['attribute'] as $product_attribute) {
								$attr_name_lower = mb_strtolower($product_attribute['name'], 'UTF-8');
								$attribute_text = $product_attribute['text'];
								
								if (strpos($attr_name_lower, 'сезон') !== false || strpos($attr_name_lower, 'season') !== false) {
									$season = $attribute_text;
								}
								if (strpos($attr_name_lower, 'розмір') !== false || strpos($attr_name_lower, 'size') !== false) {
									$sizes = $attribute_text;
								}

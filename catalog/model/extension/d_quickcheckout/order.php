...
class ModelExtensionDQuickcheckoutOrder extends Model {
    ...
    public function updateOrder($order_id, $data) {
        ...
        $order_check = $this->db->query("SELECT order_id FROM `" . DB_PREFIX . "order` WHERE order_id = '" . (int) $order_id . "'");
        if (!$order_check->num_rows) {
            // Якщо замовлення не існує, створюємо його
        ...
        foreach ($total_data['totals'] as $total) {
            if(!empty($total['title'])){
                $totals[] = array(
                    ...
                    // бо $total['value'] вже в поточній валюті (UAH)
    ...
    public function getCartTotal($total) {
        ...
        // Важливо: передаємо третій параметр $value = 1 щоб не множити на currency_value
        // бо $total вже в поточній валюті (UAH)
        return sprintf($this->language->get('text_items'), $this->cart->countProducts() + (isset($this->session->data['vouchers']) ? count($this->session->data['vouchers']) : 0), $this->currency->format($total, $this->session->data['currency'], 1));
    }
